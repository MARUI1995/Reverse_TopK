...
*/
void RTree::bbs_ranking_querypoint(float *_rslt, int &_rsltcnt, float* _qry_pt, float* weighting, int &ranking_querypoint)//_rslt是不可比点的数组，_rsltcnt是不可比点的个数，_qry_pt是查询点，weighting是目前样本权重集，ranking_querypoint是在这些不可比点中查询点的排名，最终返回给上层调用者
{
	int i, j;
	ranking_querypoint = 0;
	//---------------
	//ofstream outfile(".\\rankingResult.txt");
	//if(outfile.is_open() )
	//	cout<<"open";
	//	outfile<<"weighting: "<<weighting[0]<<"  "<<weighting[1]<<" "<<weighting[2]<<endl;
	//outfile<<"_qry_pt: "<<_qry_pt[0]<<"  "<<_qry_pt[1]<<" "<<_qry_pt[2]<<endl;
	float score_querypoint = 0.0, score_incompoint;
	for (j = 0; j < dimension; j++)
	{
		score_querypoint += weighting[j] * _qry_pt[j];
	}	//查询点在当前权重下的分数。
	//	outfile<<"score_querypoint: "<<score_querypoint<<endl;
	for (j = 0; j < _rsltcnt; j++)
	{
		...
		/*	outfile<<"_rslt[j*dimension]: "<<_rslt[j*dimension]<<"  "<<_rslt[j*dimension+1]<<" "<<_rslt[j*dimension+2]<<endl;
		outfile<<"score_incompoint: "<<score_incompoint<<endl;*/
...
void RTree::incomparable_points(Heap *_hp, float *_rslt, int &_rsltcnt, float* _qry_pt, int &bedominatnum)
{
	...
	//------------------------------------------------
	_hp->insert(he);
	//ofstream outfile(".\\incmpResult.txt");
	//outfile<<"_qry_pt  "<<_qry_pt[0]<<" "<<_qry_pt[1]<<" "<<_qry_pt[2]<<endl;
	while (_hp->used > 0)
	{
		//remove the top heap-------------------------------------
		...
		/*	outfile<<"bounces_left  "<<bounces_left[0]<<" "<<bounces_left[1]<<" "<<bounces_left[2]<<endl;
		outfile<<"bounces_right  "<<bounces_right[0]<<" "<<bounces_right[1]<<" "<<bounces_right[2]<<endl;*/
		if (level == 0)
		{
...
float* RTree::checkQueryPoint(Heap *_hp, float* _qry_pt, float* weightSet, int weightCount, int dimension, int missCnt, int* ranks, char* dataSetName)
{
	...
	//traverse_points( _hp, incompPoints,  _rsltcnt, _qry_pt, bedominatnum);
	char qFileName[100];
...
{//modifyweight, modifyk存放结果，
	...
	ofstream outfile("result_reuse1.txt", ios::app);
	outfile << "查询点: " << _qry_pt[0] << "\t" << incomnum << "\t" << bedomnum << endl;
	for (int cnt = 0; cnt < incomnum; cnt++)
	{
		for (int dim = 0; dim < dimension; dim++)
			outfile << " " << incomparable[cnt * dimension + dim];
		outfile << endl;
	}
	int qminrank;
	qminrank = bedomnum + 1;
	...
	float parameter_k, parameter_w = 0, ww = 0;
	parameter_k = 0.5 / (qmaxrank_real - k);//penalty中修正k的参数
	...
	parameter_w = 0.5 / parameter_w;//penalty中修正w的参数
